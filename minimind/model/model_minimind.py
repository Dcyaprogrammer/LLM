...
class MiniMindConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        self.seq_aux = seq_aux  # whether to calculate auxiliary loss at sequence level
        self.norm_topk_prob = norm_topk_prob  # whether to normalize top-k probability
...
def precompute_freqs_cis(dim: int, end: int = int(32 * 1024), rope_base: float = 1e6,
    ...
    if rope_scaling is not None:
        ...
        if end / orig_max > 1.0:
            ...
            # λ = (β·α - β + 1)/(β·α) YaRN standard formula
...
class Attention(nn.Module):
    ...
    def forward(self,
                x: torch.Tensor,
                position_embeddings: Tuple[torch.Tensor, torch.Tensor],  # receive cos and sin
